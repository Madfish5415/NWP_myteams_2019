    ...
    const char *team_name, const char *desc, const char *creator)
{
    ...
    team = new_node(BAD_CAST "team");
    new_text_child(team, BAD_CAST "uuid", BAD_CAST uuid_str);
    new_text_child(team, BAD_CAST "name", BAD_CAST team_name);
    new_text_child(team, BAD_CAST "desc", BAD_CAST desc);
    new_text_child(team, BAD_CAST "date", BAD_CAST time_str);
    new_text_child(team, BAD_CAST "creator", BAD_CAST creator);
    add_child(team, new_node(BAD_CAST "channels"));
    add_child(team, new_node(BAD_CAST "subscribes"));
...
exception_t team_add(xml_node_ptr team, xml_doc_ptr xml_tree)
{
    ...
    add_child(root->children->next, team);
    return exception;
}
...
xml_node_ptr team_get(xml_doc_ptr xml_tree, const char *team_uid)
{
    ...
    for (xml_node_ptr tmp = root->children->next->children; tmp;
        ...
        if (strcmp((char *)node_get_content(tmp->children), team_uid) == 0)
            return tmp;
    }
    return NULL;
}